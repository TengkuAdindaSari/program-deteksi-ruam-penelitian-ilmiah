...
DISEASE_INFO = {
    ...
    'Rubella': {
        ...
        'deskripsi' : 'Rubella (Campak Jerman) adalah infeksi virus ringan pada anak, namun '
                      'sangat berbahaya dan berisiko tinggi jika menular pada ibu hamil (menyebabkan kecacatan janin).',
...
def show_dashboard_page():
    ...
    # ── 1. Statistik Dataset ──
    st.markdown("### 🗂️ Distribusi Gambar Dataset")
    ...
    # ── 2. Grafik Training & confusion matrix ──
    ...
    with col_result:
        ...
        if predict_btn and uploaded_file:
            with st.spinner("Model sedang memproses citra dan data gejala..."):
                ...
                # Preprocessing gejala klinis (menggunakan scaler yang di-load)
                if scaler is not None:
                    durasi_norm = float(scaler.transform([[durasi_demam]])[0][0])
                else:
                    durasi_norm = (durasi_demam - 4.0) / 1.2 # fallback jika scaler tak ada
                ...
                # Detail Informasi & Penanganan Medis
